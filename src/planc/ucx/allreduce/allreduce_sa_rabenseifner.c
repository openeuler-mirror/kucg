                                                                  ...
                                                                  const ucg_coll_args_t *args)
{
    ...
    if (ppn == 1) {
        ucg_info("Allreduce sa_rabenseifner don't support ppn==1");
        return UCG_ERR_UNSUPPORTED;
    }
    ...
    if (pps == 1) {
        ucg_info("Allreduce sa_rabenseifner don't support pps==1");
        return UCG_ERR_UNSUPPORTED;
    }
    ucg_planc_ucx_group_t* ucx_group = ucg_derived_of(vgroup, ucg_planc_ucx_group_t);
    if (ucx_group->context->config.reduce_consistency == 1) {
        ucg_info("Allreduce sa_rabenseifner don't support reduce calculation results consistency");
        return UCG_ERR_UNSUPPORTED;
    }
    return UCG_OK;
}
