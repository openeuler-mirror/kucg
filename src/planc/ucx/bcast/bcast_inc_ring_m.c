...
enum {
    UCG_BCAST_INC_RING_M_SEND = UCG_BIT(0), /* send to my right */
    UCG_BCAST_INC_RING_M_RECV = UCG_BIT(1), /* receive from my left */
};

static ucg_status_t ucg_planc_ucx_bcast_inc_ring_m_op_progress(ucg_plan_op_t *ucg_op)
{
    ...
    if (ucg_test_and_clear_flags(&op->flags, UCG_BCAST_INC_RING_M_RECV)) {
        peer = ucg_algo_ring_iter_left_value(iter);
        if (myrank == recv_from_root_rank1) {
            peer = root;
        }
    ...
    if (ucg_test_and_clear_flags(&op->flags, UCG_BCAST_INC_RING_M_SEND)) {
...
static ucg_status_t ucg_planc_ucx_bcast_inc_ring_m_op_trigger(ucg_plan_op_t *ucg_op)
{
    ucg_status_t status;
    ucg_planc_ucx_op_t *op = ucg_derived_of(ucg_op, ucg_planc_ucx_op_t);
    ...
    status = ucg_planc_ucx_bcast_inc_ring_m_op_progress(ucg_op);
    return status == UCG_INPROGRESS ? UCG_OK : status;
}

static ucg_status_t ucg_planc_ucx_bcast_inc_ring_m_op_discard(ucg_plan_op_t *ucg_op)
{
                                                    ...
                                                    ucg_plan_op_t **op)
{
    ...
    ucg_status_t status;
    ucg_planc_ucx_group_t *ucx_group = ucg_derived_of(vgroup,
    ...
    status = UCG_CLASS_CONSTRUCT(ucg_plan_op_t, &ucx_op->super, vgroup,
                                 ucg_planc_ucx_bcast_inc_ring_m_op_trigger,
                                 ucg_planc_ucx_bcast_inc_ring_m_op_progress,
                                 ucg_planc_ucx_bcast_inc_ring_m_op_discard,
                                 args);
    if (status != UCG_OK) {
