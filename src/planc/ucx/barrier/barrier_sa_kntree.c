...
static ucg_status_t ucg_planc_ucx_barrier_sa_kntree_check(ucg_vgroup_t *vgroup,
                                                          const ucg_coll_args_t *args)
{
    if (vgroup->group->topo->ppn == UCG_TOPO_PPX_UNKNOWN) {
        ucg_info("Barrier sa_kntree don't support unknown ppn");
        return UCG_ERR_UNSUPPORTED;
    }
    if (vgroup->group->topo->pps == UCG_TOPO_PPX_UNKNOWN) {
        ucg_info("Barrier sa_kntree don't support unknown pps");
        return UCG_ERR_UNSUPPORTED;
    }
    return UCG_OK;
}
                                                     ...
                                                     ucg_plan_op_t **op)
{
    ...
    ucg_status_t status;
    status = ucg_planc_ucx_barrier_sa_kntree_check(vgroup, args);
    if (status != UCG_OK) {
        return UCG_ERR_UNSUPPORTED;
    }
