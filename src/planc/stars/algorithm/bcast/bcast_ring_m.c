...
#define UCG_BCAST_RING_M_RANK_OFFSET_1 1
#define UCG_BCAST_RING_M_RANK_OFFSET_2 2
#define RING_M_EID_IDX 0

static ucg_status_t ucg_planc_stars_bcast_ring_m_ofd_put_req(ucg_planc_stars_op_t *op,
                                                             stars_rank_info_h peer_rank)
{
    ...
    ucg_status_t status =
        ucg_planc_stars_fill_ofd_put_req_elem(RING_M_EID_IDX, peer_rank, request);
    UCG_ASSERT_CODE_RET(status);
    ucg_planc_stars_op_push_ofd_req_elem(op, request);
    return UCG_OK;
}

static ucg_status_t ucg_planc_stars_bcast_ring_m_ofd_wait_req(ucg_planc_stars_op_t *op,
                                                              stars_rank_info_h peer_rank)
{
    scp_ofd_req_elem_h request = ucg_planc_stars_op_get_ofd_req_elem(op);
    UCG_ASSERT_RET(request != NULL, UCG_ERR_NO_MEMORY);
    ...
    ucg_status_t status =
        ucg_planc_stars_fill_ofd_wait_req_elem(RING_M_EID_IDX, peer_rank, request, op->plan.event_elem);
...
static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, submit_op)(ucg_planc_stars_op_t *op)
{
    ...
    for (uint32_t idx = 0; idx < plan->comm_dep.get_rank_num; ++idx) {
        ...
        status = ucg_planc_stars_bcast_ring_m_ofd_wait_req(op, peer_rank);
    ...
    for (uint32_t idx = 0; idx < plan->comm_dep.put_rank_num; ++idx) {
        ...
        status = ucg_planc_stars_bcast_ring_m_ofd_put_req(op, peer_rank);
...
static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, init_rbuf)(ucg_planc_stars_op_t *op,
                                            ucg_coll_args_t *coll_args)
{
    stars_comm_plan_t *plan = &op->plan;
    ucg_coll_bcast_args_t *args = &coll_args->bcast;
    if (op->super.vgroup->myrank == args->root) {
...
static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, init_sbuf)(ucg_planc_stars_op_t *op,
                                            ucg_coll_args_t *coll_args)
{
    stars_comm_plan_t *plan = &op->plan;
    ucg_coll_bcast_args_t *args = &coll_args->bcast;
    ...
    if (ucg_unlikely(op->super.vgroup->myrank != args->root)) {
        /* bcast ringm have one buf, root rank used
           to send data, then non-root rank used to recv */
...
UCG_STARS_ALGO_FUN(bcast_ring_m, put_max_size)(ucg_planc_stars_op_t *op,
                                               ucg_coll_args_t *coll_args)
{
    ucg_coll_bcast_args_t *args = &coll_args->bcast;
...
static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, offload_plan)(ucg_planc_stars_op_t *op)
{
    ...
    if ((myid == (root - 1 + numprocs) % numprocs) ||
        (myid == (root + UCG_BCAST_RING_M_RANK_OFFSET_1) % numprocs)) {
    ...
    for (uint32_t cnt = 0; cnt < comm_dep->get_rank_num; ++cnt) {
        peer_id = (myid == (root + UCG_BCAST_RING_M_RANK_OFFSET_2) % numprocs) ?
                  root : (myid - 1 + numprocs) % numprocs;
...
UCG_STARS_ALGO_FUN(bcast_ring_m, check)(ucg_vgroup_t *vgroup,
                                        const ucg_coll_args_t *coll_args) {
    if (ucg_unlikely(vgroup->size <= 2)) {
        ucg_info("ring-Modified don't support the number of progresses less than 3");
...
static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, init)(ucg_plan_op_t *ucg_op)
{
    ...
    status = ucg_planc_stars_algo_prepare_plan(op, coll_args,
                                               UCG_STARS_ALGO_FUN(bcast_ring_m, put_max_size),
                                               UCG_STARS_ALGO_FUN(bcast_ring_m, offload_plan));
    UCG_CHECK_GOTO_ERR(status, out, "prepare bcast ring modify plan");

    status = ucg_planc_stars_algo_exch_buf_addr(op, coll_args,
                                                UCG_STARS_ALGO_FUN(bcast_ring_m, init_rbuf),
                                                UCG_STARS_ALGO_FUN(bcast_ring_m, init_sbuf));
...
static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, trigger)(ucg_plan_op_t *ucg_op)
{
    ucg_status_t status;
    ...
    status = UCG_STARS_ALGO_FUN(bcast_ring_m, submit_op)(op);
    UCG_ASSERT_CODE_RET(status);
...
inline static ucg_status_t UCG_STARS_ALGO_FUN(bcast_ring_m, progress)(ucg_plan_op_t *ucg_op)
{
    return ucg_planc_stars_algo_progress(ucg_op, NULL);
}

static ucg_status_t
UCG_STARS_ALGO_FUN(bcast_ring_m, discard)(ucg_plan_op_t *ucg_op)
{
    UCG_STATS_GET_TIME(start_tick);
    ucg_planc_stars_op_t *op = ucg_derived_of(ucg_op, ucg_planc_stars_op_t);\
    ...
    ucg_stats_dump("bcast_ring_m", &op->stats);
    return UCG_OK;
}

UCG_STARS_ALGO_PRE_DEFINE(bcast_ring_m)
