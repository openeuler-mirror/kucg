 ...
 * @param elem   Element to add.
 */
static inline void ucg_queue_push_head(ucs_queue_head_t *queue,
 ...
 */
static inline ucs_queue_elem_t *ucg_queue_pull_non_empty(ucs_queue_head_t *queue)
{
    ucs_queue_elem_t *elem;
...
{
    ucg_assert((iter != NULL) && (*iter != NULL));
...
static inline ucs_queue_elem_t *ucs_queue_pull(ucs_queue_head_t *queue)
{
    if (ucs_queue_is_empty(queue))
        return NULL;
    return ucg_queue_pull_non_empty(queue);
}
...
#define ucs_queue_pull_elem_non_empty(queue, type, member) \
    ucs_container_of(ucg_queue_pull_non_empty(queue), type, member)
...
#define ucs_queue_for_each_extract(elem, queue, member, cond) \
    for (elem = ucs_container_of((queue)->head, typeof(*elem), member); \
         \
         !ucs_queue_is_empty(queue) && (cond) && ucg_queue_pull_non_empty(queue); \
         \
         elem = ucs_container_of((queue)->head, typeof(*elem), member))
