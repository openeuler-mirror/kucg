...
TEST_F(test_planc_hccl_config, read_user_env_prefix)
{
    ...
    ASSERT_EQ(ucg_planc_hccl_config_read("GTEST", NULL, &config), UCG_OK);
...
TEST_F(test_planc_hccl_config, read_invalid_args)
{
    ...
    ASSERT_EQ(ucg_planc_hccl_config_read(NULL, NULL, NULL), UCG_ERR_INVALID_PARAM);
    // filename is unsupported now
    ASSERT_EQ(ucg_planc_hccl_config_read(NULL, "unsupported_param", &config), UCG_ERR_UNSUPPORTED);
}
#endif
...
TEST_F(test_planc_hccl_config, modify_non_exist)
{
    ...
    ASSERT_NE(ucg_planc_hccl_config_modify(config, "NON_EXIST", "value"), UCG_OK);
...
class test_planc_hccl_context : public testing::Test {
public:
    static void SetUpTestSuite()
    {
        stub::init();
...
ucg_planc_config_h test_planc_hccl_context::m_config = NULL;
ucg_context_t test_planc_hccl_context::m_ucg_context;
...
TEST_F(test_planc_hccl_context, init_fail_malloc)
{
    ...
    ucg_planc_params_t params;
    std::vector<stub::routine_result_t> result = {stub::FAILURE};
    stub::mock(stub::CALLOC, result, "planc hccl context");
