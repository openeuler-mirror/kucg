...
TEST_F(test_ucg_planc_load, multi_times)
{
    ASSERT_EQ(ucg_planc_load(), UCG_OK);
...
TEST_F(test_ucg_planc_load, fail_malloc)
{
    ...
    stub::mock(stub::STRDUP, result, "planc lib path");
    ASSERT_EQ(ucg_planc_load(), UCG_ERR_NO_RESOURCE);

    stub::mock(stub::MALLOC, result, "planc default path");
    ASSERT_EQ(ucg_planc_load(), UCG_ERR_NO_RESOURCE);
}
#endif
...
{
    int invalid_idx = ucg_planc_count();
