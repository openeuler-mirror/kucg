...
TEST_F(test_ucg_rank_map, init_by_array_invalid_args)
{
    ucg_rank_map_t map;
    const int size = 4;
...
TEST_F(test_ucg_rank_map, copy_invalid_args)
{
    ucg_rank_map_t dst;
    ucg_rank_map_t src;
...
TEST_F(test_ucg_rank_map, cleanup_invalid_args)
{
    // except no segmentation fault
    ucg_rank_map_cleanup(NULL);
}

TEST_F(test_ucg_rank_map, eval_invalid_args)
{
...
TEST_F(test_ucg_rank_map, init_by_array)
{
    ucg_rank_map_t map;
    ...
    // rank array that can be optimized
    ranks = (ucg_rank_t*)malloc(size * sizeof(ucg_rank_t));
    for (int i = 0; i < size; ++i) {
        ranks[i] = i;
    }
...
TEST_F(test_ucg_rank_map, init_by_array_fail_malloc)
{
    int size = 6;
    ucg_rank_t *ranks = (ucg_rank_t*)malloc(size * sizeof(ucg_rank_t));
    for (int i = 0; i < size; ++i) {
...
TEST_F(test_ucg_rank_map, optimize_full)
{
    ucg_rank_t *ranks;
    ucg_rank_map_t map;
...
TEST_F(test_ucg_rank_map, optimize_array)
{
    int size = 6;
    ucg_rank_t *ranks = (ucg_rank_t*)malloc(size * sizeof(ucg_rank_t));
    ...
    ASSERT_EQ(map.strided.stride, stride);
    ...
    map.array = ranks;
    ucg_rank_map_optimize(&map, &original_ranks);
    ASSERT_TRUE(original_ranks == NULL);
    ASSERT_EQ(map.type, UCG_RANK_MAP_TYPE_ARRAY);
    ASSERT_EQ(map.size, size);
    for (int i = 0; i < size; ++i) {
        ASSERT_EQ(map.array[i], i * i);
...
TEST_F(test_ucg_rank_map, optimize_stride)
{
    ucg_rank_t *ranks;
    ucg_rank_map_t map;
    ...
    // stride is not 1, cannot optimize
    map.type = UCG_RANK_MAP_TYPE_STRIDE;
    map.strided.start = 0;
...
TEST_F(test_ucg_rank_map, optimize_cb)
{
    ucg_rank_t *ranks;
    ucg_rank_map_t map;
...
TEST_F(test_ucg_rank_map, copy)
{
    ...
    src.array = ranks;
    ASSERT_EQ(ucg_rank_map_copy(&dst, &src), UCG_OK);
    for (int i = 0; i < size; ++i) {
...
TEST_F(test_ucg_rank_map, eval_unknow_type)
{
    ucg_rank_map_t map;
