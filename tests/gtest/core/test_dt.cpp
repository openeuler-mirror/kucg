...
typedef struct {
    uint32_t data1;
    uint64_t data2;
} __attribute__((packed, aligned(1))) contig_dt_t;

typedef struct {
    uint32_t data1;
    uint16_t hole1;
    uint64_t data2;
} __attribute__((packed, aligned(1))) non_contig_dt_t;
...
static contig_dt_t g_contig_dt;
...
public:
    static void SetUpTestSuite()
    {
        ...
        params.extent = sizeof(non_contig_dt_t);
        params.conv = g_conv;
        ...
        params.user_dt = &g_non_contig_dt;
        params.size = sizeof(contig_dt_t);
        params.extent = sizeof(contig_dt_t);
        ucg_dt_create(&params, &m_ucg_contig_dt);
    }
    ...
    static ucg_dt_h m_ucg_contig_dt;
};
...
#define CHECK_ALL(_ucg_op_type, _expect_op) \
    do { \
        ...
        CHECK(uint8_t, _ucg_op_type, UCG_DT_TYPE_UINT8, source, target, expect, size, _expect_op); \
...
public:
    static void SetUpTestSuite()
    {
        ucg_dt_global_init();
        {
            ucg_dt_params_t params;
            params.field_mask = UCG_DT_PARAMS_FIELD_TYPE;
            for (int i = 0; i < UCG_DT_TYPE_PREDEFINED_LAST; ++i) {
...

        {
            ...
            for (int i = 0; i < UCG_OP_TYPE_PREDEFINED_LAST; ++i) {
    ...
    static ucg_op_h m_ucg_op_predefined[UCG_OP_TYPE_PREDEFINED_LAST];
...
TEST(test_ucg_dt_create, predefined)
{
    ucg_dt_h dt;
    ucg_dt_params_t params;
    params.field_mask = UCG_DT_PARAMS_FIELD_TYPE;
...
TEST(test_ucg_dt_create, predefined_multi)
{
    ucg_status_t status;
    ucg_dt_h dt1;
    ucg_dt_h dt2;
...
TEST(test_ucg_dt_create, user_contiguous)
{
    ucg_dt_h dt;
    ucg_dt_params_t params;
    params.field_mask = UCG_DT_PARAMS_FIELD_TYPE |
...
TEST(test_ucg_dt_create, user_non_contiguous)
{
    ucg_dt_h dt;
    ucg_dt_params_t params;
    params.field_mask = UCG_DT_PARAMS_FIELD_TYPE |
...
TEST_F(test_ucg_dt, pack_contig)
{
    const int count = 12;
    ...
    for (int i = 0; i < count; ++i){
        ...
        ASSERT_EQ(status, UCG_OK);
        ASSERT_EQ(len, sizeof(contig_dt_t));
...
TEST_F(test_ucg_dt, pack_non_contig)
{
    const int count = 12;
    non_contig_dt_t non_contig_value[count];
    for (int i = 0; i < count; ++i) {
        non_contig_value[i].data1 = i;
        non_contig_value[i].data2 = i + 1;
    }
...
TEST_F(test_ucg_dt, unpack_contig)
{
    const int count = 12;
...
TEST_F(test_ucg_dt, unpack_non_contig)
{
    const int count = 12;
    non_contig_dt_t non_contig_value[count] = {{0}};
    ucg_dt_state_t *state = ucg_dt_start_unpack(&non_contig_value, m_ucg_non_contig_dt, count);
    ASSERT_TRUE(state != NULL);
    contig_dt_t contig_value[count];
    for (int i = 0; i < count; ++i) {
        contig_value[i].data1 = i;
...
TEST_F(test_ucg_dt, memcpy_contiguous)
{
    ucg_status_t status;
    const int count = 12;
...
TEST_F(test_ucg_dt, memcpy_contiguous_truncate)
{
    ucg_status_t status;
    const int count = 12;
    contig_dt_t src_value[count] = {{0}};
...
TEST_F(test_ucg_dt, memcpy_pack)
{
    ucg_status_t status;
    const int count = 12;
...
TEST_F(test_ucg_dt, memcpy_pack_truncate)
{
    ucg_status_t status;
    const int count = 12;
    non_contig_dt_t src_value[count] = {{0}};
...
TEST_F(test_ucg_dt, memcpy_unpack)
{
    ucg_status_t status;
    const int count = 12;
    contig_dt_t src_value[count] = {{0}};
...
TEST_F(test_ucg_dt, memcpy_unpack_truncate)
{
    ucg_status_t status;
    const int count = 12;
    contig_dt_t src_value[count] = {{0}};
...
TEST_F(test_ucg_dt, memcpy_generic)
{
    ucg_status_t status;
    const int count = 12;
    non_contig_dt_t src_value[count] = {{0}};
...
TEST_F(test_ucg_dt, memcpy_generic_truncate)
{
    ucg_status_t status;
    const int count = 12;
    non_contig_dt_t src_value[count] = {{0}};
...
TEST(test_ucg_op_create, predfined)
{
    ucg_op_h op;
    ucg_op_params_t params;
    params.field_mask = UCG_OP_PARAMS_FIELD_TYPE;
    for (int i = 0; i < UCG_OP_TYPE_PREDEFINED_LAST; ++i) {
        ...
        ASSERT_EQ(status, UCG_OK);
        ASSERT_TRUE(ucg_op_is_predefined(op));
        ASSERT_TRUE(ucg_op_is_commutative(op));
        ucg_op_destroy(op);
    }
}

TEST(test_ucg_op_create, predfined_multi)
{
    ucg_status_t status;
    ucg_op_h op1;
    ucg_op_h op2;
...
{
    CHECK_ALL(UCG_OP_TYPE_SUM, sum);
}

TEST_F(test_ucg_op, prod)
{
    CHECK_ALL(UCG_OP_TYPE_PROD, prod);
}
