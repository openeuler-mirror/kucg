...
static void check_expect_data(ucg_list_link_t *list, std::vector<test_expect_plan_data> &expect)
{
...
TEST(test_ucg_plan, add_same_score)
{
    ...
    std::vector<ucg_plan_params_t> params = {
        ...
        {mem_type, coll_type, {prepare_ok, 0, "", "", 0, {1000, 4000}, VGRP_PTR(101), 10}},
    };
    ASSERT_EQ(ucg_plans_init(&plans), UCG_OK);
...
TEST(test_ucg_plan, add_plan_in_different_order)
{
    ...
    ucg_list_link_t *head1 = &plans1->plans[coll_type][mem_type];
    ucg_list_link_t *head2 = &plans2->plans[coll_type][mem_type];
...
TEST(test_ucg_plan_attr_update, score_invalid_fmt)
{
    ...
    ASSERT_EQ(ucg_plan_attr_update(&attr, update), UCG_ERR_INVALID_PARAM);
}
