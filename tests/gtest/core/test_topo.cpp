...
static const int32_t pps = ppn / n_sockets; // prcesses per socket
...
TEST_F(test_ucg_topo, init_invalid_args)
{
...
TEST_F(test_ucg_topo, cleanup_invalid_args)
{
    // expect no segmentation fault.
    ucg_topo_cleanup(NULL);
}

TEST_F(test_ucg_topo, get_group_invalid_args)
{
    // NULL topo
    ...
    ASSERT_TRUE(ucg_topo_get_group(topo, UCG_TOPO_GROUP_TYPE_LAST) == NULL);
}
#endif

TEST_F(test_ucg_topo, init)
{
    ucg_rank_map_t map;
...
TEST_F(test_ucg_topo, init_fail_malloc)
{
    ucg_topo_t *topo;
    ucg_rank_map_t map;
    ucg_topo_params_t params;
...
TEST_F(test_ucg_topo, init_fail_location)
{
    ucg_topo_t *topo;
...
TEST_F(test_ucg_topo, get_socket_group_only)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = n_proc;
    ...
    topo_group = ucg_topo_get_group(topo, UCG_TOPO_GROUP_TYPE_SOCKET);
    ASSERT_TRUE(topo_group != NULL);
    ucg_topo_cleanup(topo);
}

TEST_F(test_ucg_topo, get_socket_leader_group_only)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = n_proc;
    ...
    ucg_topo_group_t *topo_group;
    topo_group = ucg_topo_get_group(topo, UCG_TOPO_GROUP_TYPE_SOCKET_LEADER);
    ASSERT_TRUE(topo_group != NULL);
    ucg_topo_cleanup(topo);
}

TEST_F(test_ucg_topo, get_group_fail_location)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = 10;
    ...
    ucg_topo_group_t *topo_group;
    topo_group = ucg_topo_get_group(topo, UCG_TOPO_GROUP_TYPE_NODE);
    ASSERT_TRUE(topo_group == NULL);
...
TEST_F(test_ucg_topo, get_group_no_node_id)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = 10;
...
TEST_F(test_ucg_topo, get_group_no_socket_id)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = 10;
...
#ifdef UCG_ENABLE_DEBUG
TEST_F(test_ucg_topo, get_group_fail_malloc)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = n_proc;
    ...
    stub::mock(stub::REALLOC, {stub::FAILURE}, "ucg array extend");
    topo_group = ucg_topo_get_group(topo, UCG_TOPO_GROUP_TYPE_NODE_LEADER);
    ASSERT_TRUE(topo_group == NULL);

    // The initial capacity of ranks is 256. However, there are 10240 processes.
    stub::mock(stub::REALLOC, {stub::FAILURE}, "ucg arrayx extend");
...
#ifdef UCG_ENABLE_DEBUG
TEST_F(test_ucg_topo, get_group_fail_malloc_and_retry)
{
    ucg_rank_map_t map;
    map.type = UCG_RANK_MAP_TYPE_FULL;
    map.size = n_proc;
...
TEST_F(test_ucg_topo_get_group, net)
{
...
TEST_F(test_ucg_topo_get_group, subnet)
{
    ucg_topo_group_t *group;
    for (int i = 0; i < n_proc; ++i) {
...
TEST_F(test_ucg_topo_get_group, subnet_leader)
{
    ucg_topo_group_t *group;
    ...
    for (int i = 0; i < n_proc; i += 45) {
        ...
        if (i % (n_proc / n_subnet) == 0) {
            ASSERT_EQ(group->state, UCG_TOPO_GROUP_STATE_ENABLE);
            ASSERT_EQ(group->super.size, n_subnet);
            ASSERT_EQ(group->super.myrank, i / (n_proc / n_subnet));
            ASSERT_EQ(ucg_rank_map_eval(&group->super.rank_map, group->super.myrank), i);
        } else {
...
TEST_F(test_ucg_topo_get_group, node)
{
    ucg_topo_group_t *group;
    for (int i = 0; i < n_proc; ++i) {
...
TEST_F(test_ucg_topo_get_group, node_leader)
{
    ucg_topo_group_t *group;
    // Iterating all processes takes too much time, so set stride to 45.
    // Some of the selected processes are leaders, while others are not.
...
TEST_F(test_ucg_topo_get_group, socket)
{
    ucg_topo_group_t *group;
    for (int i = 0; i < n_proc; ++i) {
        ...
        ASSERT_TRUE(group != NULL);
        ASSERT_EQ(group->state, UCG_TOPO_GROUP_STATE_ENABLE);
        ASSERT_EQ(group->super.size, pps);
        ucg_rank_t node_rank = i - i / ppn * ppn;
...
TEST_F(test_ucg_topo_get_group, socket_leader)
{
    ucg_topo_group_t *group;
    for (int i = 0; i < n_proc; ++i) {
