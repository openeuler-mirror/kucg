...
ucg_config_h test_ucg_context::m_config = NULL;

TEST_F(test_ucg_config, read_default_config)
{
    ucg_config_h config;
...
TEST_F(test_ucg_config, read_user_config)
{
    ucg_config_h config;
...
TEST_F(test_ucg_config, read_user_env_prefix)
{
    ucg_config_h config;
...
TEST_F(test_ucg_config, read_invalid_args)
{
    ucg_config_h config;
...
TEST_F(test_ucg_config, modify)
{
    ...
    ASSERT_STREQ(config->planc.names[0], "ucx");
    ASSERT_STREQ(config->planc.names[1], "hccl");
...
#ifdef UCG_ENABLE_CHECK_PARAMS
TEST_F(test_ucg_config, modify_invalid_args)
{
    ucg_config_h config;
    ASSERT_EQ(ucg_config_read(NULL, NULL, &config), UCG_OK);
...
TEST_F(test_ucg_config, modify_non_exist)
{
    ucg_config_h config;
    ASSERT_EQ(ucg_config_read(NULL, NULL, &config), UCG_OK);
...
#ifdef UCG_ENABLE_CHECK_PARAMS
TEST_F(test_ucg_config, release_invalid_args)
{
...
#ifdef UCG_ENABLE_CHECK_PARAMS
TEST_F(test_ucg_context, init_invalid_args)
{
...
TEST_F(test_ucg_context, init_fail_no_required_planc)
{
    ...
    ucg_context_h context;
    ASSERT_NE(ucg_init(&test_stub_context_params, config, &context), UCG_OK);

    ucg_config_release(config);
}

#ifdef UCG_ENABLE_CHECK_PARAMS
TEST_F(test_ucg_context, cleanup_invalid_args)
{
    // expect no segmentation fault.
    ucg_cleanup(NULL);
}
